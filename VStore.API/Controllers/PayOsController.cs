...
public class PayOsController(ISender sender, IPayOsService payOsService) : ApiController(sender)
{
    ...
    public async Task<IActionResult> VerifyPayOsWebHook([FromBody] WebhookType data)
    {
        ...
        var res = await payOsService.VerifyPaymentWebHookType(webhookType);
    ...
    {
        var res = await payOsService.VerifyPaymentWebHook(model);
        return res.IsSuccess ? Ok(res.Value) : BadRequest(res.Error);
    }
    ...
    {
        var res = await payOsService.CreateSignatureWebHook(data);
        return res.IsSuccess ? Ok(res.Value) : BadRequest(res.Error);
    }
    ...
    {
        var res = await payOsService.GetPaymentInformation(orderCode);
        return res.IsSuccess ? Ok(res.Value) : BadRequest(res.Error);
    }
}